...
class PaymentService {
    ...
    constructor(params) {
        ...
        this._makePassword = function (timestamp) {
            ...
            return (new Buffer(crypto.createHash('sha256').update([options.merchant,options.passkey,timestamp].join['']).digest('hex'))).toString('base64');
        };
                ...
                switch (op) {
                    case OPERATIONS.PAYMENT_RESULT:
                        ...
                        data = { // FIXME: Seems to be same as OPERATIONS[2]
                            code: parseInt(response['RETURN_CODE']['$t']),
                            description: response['DESCRIPTION']['$t'],

                            account: response['MSISDN']['$t'],
                            amount: response['AMOUNT']['$t'],
                            date: !!response['M-PESA_TRX_DATE'] ? response['M-PESA_TRX_DATE']['$t'] : null, // In case of failure
                            status: response['TRX_STATUS']['$t'],
                            mpesa_txn: response['TRX_ID']['$t'],
                            internal_txn: !!response['M-PESA_TRX_ID'] ? response['M-PESA_TRX_ID']['$t'] : null, // In case of failure
                            transaction: response['MERCHANT_TRANSACTION_ID']['$t'],
                            details: response['ENC_PARAMS']['$t']
                        };
                        break;
     ...
     * @param next
     */
    paymentNotification(referenceToInstance) {
        return (req, res, next) => {

            let xml = req.body; // GET params? Plain POST? XML POST?

            let data = referenceToInstance._parseXMLResponse(OPERATIONS.PAYMENT_RESULT, xml);

            if (data.error) {
                console.error(data.error);

                next(!!data.code ? new Error(data.error.description) : data.error);
            } else {
                req.payment = data;
                if (next) {
                    next(); // http://stackoverflow.com/questions/27831550/node-js-express-next-callbacks
                } else {
                    res.send('ok');
                }
            }
